...
local m = mqtt.Client("nodemcu_temp01", 60, "liang", "peng")
...
m:on("connect", function(client) print("connected , heap size:"..node.heap()) end)
...
m:connect("114.215.120.146", 1883, 0, function(client)
        ...
        tmr.alarm(1, 2000, tmr.ALARM_AUTO, function() 
            local data = get_temp()
            client:publish("/temp", sjson.encode(data), 0, 0, function(client) end)
        end)
