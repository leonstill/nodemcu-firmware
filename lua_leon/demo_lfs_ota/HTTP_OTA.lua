...
-- executing LFS.HTTP_OTA('your server', 'port', 'directory','image name').  Note
...
-- Adding suppport of "hostname:port"
-- modified by leonstill
-- 2018.12.01

local host, dir, image = ...
local port = 80

if not image then error ("Invalid param for 'image'!", 3) end
-- parsing host -> <hostname|ip>[:port]
local i,_ = string.find(host, ':')
if i then 
